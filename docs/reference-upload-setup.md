...
The website code accepts up to three JPG, PNG or WebP reference images of 1.5 MiB each after a paid-order design brief has been saved. The upload token is signed for the confirmed order and expires after one hour. Images are kept in a private Google Drive folder; the References sheet records their file IDs. The customer can still use WhatsApp when uploads are unavailable.
...
5. Submit a real test design brief, upload one small reference image, and check that its order ID and private Drive file ID appear in the References sheet. Test a rejected fourth image and an unavailable upload response. Remove the test order and image afterward as appropriate.
