...
const now = Date.now(), token = issueToken('ORD-1001', process.env.LEAD_SHARED_SECRET, now);
assert.ok(verifyToken(token, 'ORD-1001', process.env.LEAD_SHARED_SECRET, now));
assert.equal(verifyToken(token, 'ORD-1002', process.env.LEAD_SHARED_SECRET, now), false);
assert.equal(verifyToken(token + 'x', 'ORD-1001', process.env.LEAD_SHARED_SECRET, now), false);
assert.equal(verifyToken(token, 'ORD-1001', process.env.LEAD_SHARED_SECRET, now + 3600001), false);
const png = Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]), Buffer.alloc(24)]).toString('base64');
const body = { order_id:'ORD-1001', upload_token:token, filename:'reference.png', mime:'image/png', data:png };
let forwarded;
...
assert.equal(forwarded.order_id, 'ORD-1001');
...
const briefResult = await invoke(brief, { method:'POST', body:{ order_id:'ORD-1001', mobile:'9876543210', notes:'Design brief' }, headers:{} });
assert.equal(briefResult.statusCode, 200);
assert.ok(verifyToken(briefResult.payload.upload_token, 'ORD-1001', process.env.LEAD_SHARED_SECRET));
global.fetch = async () => ({ ok:true, json:async()=>({ok:true,order:{status:'Payment Failed'}}) });
const unpaid = await invoke(brief, { method:'POST', body:{order_id:'ORD-1001',mobile:'9876543210',notes:'Not paid'},headers:{} });
