...
module.exports = async (req, res) => {
  ...
  const { order_id: orderId, upload_token: token, mime, data, filename } = body;
  if (!verifyToken(token, orderId, secret)) return reply(403, 'invalid_upload_token');
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ action: 'reference_upload', secret, order_id: orderId, filename: cleanName, mime, data }),
